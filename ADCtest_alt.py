...
def sendADCcmd(CMDvar):
    ...
    spi.mode = mode
    if CMDvar == 'RDATA':
        datalist = [CMD_ADC[CMDvar]]
        datalist.extend(zeros(numbytes))
        data = spi.xfer2(datalist)
        data.pop(0)
        #print(data)
...
def ADCReadAllReg():
    try:
        ...
        regs = spi.xfer2([CMD_ADC['RREG'] + REGADDR_ADC['ID'], 0x0F, 0x00, 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00]) #Read ll 16 registers starting with "ID"
        regs.pop(0)
        regs.pop(1)
        ...
        print("Could not read ADC registers")
...
def readregs_ADS131E08(REGvar, bytenum): #Check if byte pairs need to be split in half
...
def writeregs_ADS131E08(REGvar, DATAvar):
    try:    
        spi.open(0,spiaddr_ADS131E08)
        spi.mode = mode
        spi.max_speed_hz = clockrate
